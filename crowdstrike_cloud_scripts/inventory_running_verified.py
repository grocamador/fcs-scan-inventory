#!/usr/bin/env python3
"""
VERIFIED: Running Cloud Instances Query - Using correct instance_state values per cloud
Focus ONLY on instance_state:"VM running" for Azure (as requested)
"""
...
def query_running_instances(credentials: Dict[str, str]) -> Tuple[int, int, int]:
    ...
    VERIFIED Status Field Mapping:
    - AWS EC2: instance_state:"running" (accurate)
    - GCP: active:true (instance_state not populated)
    - Azure: instance_state:"VM running" (MUST use "VM running", not just "running")
    ...
    try:
        ...
        print("🔍 Querying running EC2 instances (instance_state:\"running\")...")
        ...
        # Query for running GCP instances
        print("🔍 Querying running GCP instances (resource_type_name:\"Compute Instance\"+instance_state:\"RUNNING\")...")
        ...
        print("🔍 Querying running Azure instances (instance_state:\"VM running\")...")
...
def main():
    """Main entry point."""
    print("🚀 CrowdStrike Running Instances Query Tool (VERIFIED)")
    print("-" * 55)
